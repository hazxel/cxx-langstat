...
auto ClassMethodCallTranslationUnitAnalysis::deduplicator_ = Deduplicator();

// FIXME:
// Use nontrivial destructor if necessary
ClassMethodCallTranslationUnitAnalysis::ClassMethodCallTranslationUnitAnalysis() :
    ClassMethodCallTranslationUnitAnalysis(anything(), ".*") {}
...
void ClassMethodCallTranslationUnitAnalysis::extractFeatures(){
...
// for method calls
void ClassMethodCallTranslationUnitAnalysis::gatherData(const Matches<clang::CXXMemberCallExpr>& matches){
    ordered_json js;
...
void ClassMethodCallTranslationUnitAnalysis::gatherData(const Matches<clang::VarDecl>& matches) {
    ordered_json js;
...
// for function calls
void ClassMethodCallTranslationUnitAnalysis::gatherData(const Matches<clang::CallExpr>& matches) {
    ordered_json js;
...
void ClassMethodCallTranslationUnitAnalysis::analyzeFeatures(){
    extractFeatures();
...
void ClassMethodCallTranslationUnitAnalysis::processFeatures(nlohmann::ordered_json j){
...
void ClassMethodCallTranslationUnitAnalysis::funcPrevalence(const ordered_json& in, ordered_json& res){
    std::map<std::string, unsigned> m;
    for (auto& [func_name, func_list] : in.items()) {
        for (auto& func : func_list) {
            if (deduplicator_.isDuplicated(func_name + func[feature_file_name_key_].get<string>())) {
                continue;
            }
...
void ClassMethodCallTranslationUnitAnalysis::methodPrevalence(const ordered_json& in, ordered_json& res){
    std::map<std::string, std::map<std::string, unsigned>> m;
    for (auto& [type_name, func_list] : in.items()) {
        for (auto& func : func_list) {
            if (deduplicator_.isDuplicated(type_name + func[feature_method_name_key_].get<string>() + func[feature_file_name_key_].get<string>())) {
                continue;
            }
...
void ClassMethodCallTranslationUnitAnalysis::constructorPrevalence(const ordered_json& in, ordered_json& res){
    std::map<std::string, int> m;
    for (auto& [type_name, call_list] : in.items()) {
        for (auto& call : call_list) {
            if (deduplicator_.isDuplicated(type_name + call[feature_file_name_key_].get<string>())) {
                continue;
            }
            m[type_name]++;
        }
